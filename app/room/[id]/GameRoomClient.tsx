  ...
  return (
    <div style={{ position: 'relative', width: 64, height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <span style={{
        ...
        fontSize: 20, color: danger ? '#EC4899' : '#1B1830',
...
const HiddenWord = ({ hint, wordLength, drawerName, isDrawer, word }: {
  ...
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
    ...
    <span style={{ fontFamily: "'Caveat', cursive", color: '#5A5275', fontSize: 14, marginTop: 4 }}>
      {wordLength} letters
    </span>
  </div>
)
      ...
      {sorted.map((p, idx) => {
        ...
        return (
          <div key={p.id} style={{
            ...
            <span style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: '#312E81', fontSize: 14 }}>
              {scores[p.id] || 0}
            </span>
          </div>
        )
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {messages.map(m => (
          <div key={m.id} style={{
            ...
            {m.type !== 'system' && (
              <strong style={{ color: '#1B1830', marginRight: 4 }}>@{m.username}</strong>
            )}
            {m.message}
          </div>
        ))}
...
const DrawToolbar = ({ color, setColor, brushSize, setBrushSize, onUndo, onClear, tool, setTool }: {
  ...
  <div style={{
    ...
    border: '1px solid rgba(255,255,255,0.65)',
    boxShadow: '0 2px 12px rgba(31,27,92,0.1)',
    ...
    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', maxWidth: 180 }}>
      {COLORS.map(c => (
        <button key={c} onClick={() => { setColor(c); setTool('brush') }} style={{
          ...
          background: c, border: `2px solid ${color === c ? '#312E81' : c === '#FFFFFF' ? 'rgba(27,24,48,0.2)' : 'transparent'}`,
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      {BRUSH_SIZES.map(s => (
        <button key={s} onClick={() => setBrushSize(s)} style={{
          ...
          borderRadius: '50%',
          background: brushSize === s ? '#312E81' : '#1B1830',
          border: 'none', cursor: 'pointer',
          opacity: brushSize === s ? 1 : 0.25,
          transition: 'all 0.15s',
        }} />
      ))}
    </div>
...
// ─── Canvas Component ─────────────────────────────────────────────────────────
const DrawingCanvas = ({ isDrawer, onDraw, onClear, externalDraw, externalClear }: {
  isDrawer: boolean
  ...
  // Handle incoming draw events from other players
useEffect(() => {
  if (!externalDraw || isDrawer) return
  const ctx = getCtx()
  if (!ctx) return

  // ✅ ADD THIS LINE — ignore end events, they carry no coordinates
      ...
      <div style={{
        ...
        {!isDrawer && (
          <div style={{
            ...
            fontSize: 11, color: '#5A5275', fontWeight: 600,
          }}>
            👁 spectating
          </div>
        )}
...
const RevealOverlay = ({ word, scores, players }: { word: string; scores: Record<string, number>; players: Player[] }) => (
  <div style={{
    ...
    <div style={{
      background: '#FBF6EC', borderRadius: 24, padding: '40px 48px',
      textAlign: 'center', maxWidth: 400,
      boxShadow: '0 24px 64px rgba(31,27,92,0.3)',
    }}>
      <div style={{ fontSize: 48, marginBottom: 12 }}>🎨</div>
      <div style={{ fontFamily: "'Caveat', cursive", color: '#5A5275', fontSize: 20, marginBottom: 4 }}>the word was</div>
      <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: '#312E81', fontSize: 40, marginBottom: 20 }}>
        {word}
      </div>
...
export default function GameRoomClient({ roomId, user }: Props) {
  ...
  const [myWord, setMyWord] = useState('') // only set for the drawer
  ...
  useEffect(() => {
    ...
    socket.on('draw:stroke', (event: DrawEvent) => {
      setExternalDraw({ ...event })
    })
    ...
    socket.on('guess:correct', ({ points, word }) => {
      setMessages(prev => [...prev, {
        ...
        userId: user.userId,
        username: user.username,
        message: `✓ You guessed it! "${word}" — +${points} pts`,
        type: 'correct',
      }])
    })
    ...
    <div style={{
      ...
      <header style={{ flexShrink: 0, padding: '12px 20px' }}>
        <div style={{
          ...
          {phase === 'waiting' && (
            <div style={{ fontFamily: "'Caveat', cursive", color: '#5A5275', fontSize: 20 }}>
              waiting for players…
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button onClick={handleLeave} style={{
              ...
              fontSize: 13, fontWeight: 600, color: '#5A5275',
      ...
      <main style={{ flex: 1, padding: '0 20px 20px', display: 'grid', gridTemplateColumns: '200px 1fr 220px', gap: 12, minHeight: 0 }}>
        ...
        <div style={{ position: 'relative', display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Overlays */}
          {phase === 'reveal' && <RevealOverlay word={revealWord} scores={scores} players={players} />}
