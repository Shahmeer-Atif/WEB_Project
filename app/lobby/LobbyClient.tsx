...
const TopNav = ({ user, onLogout, onFriendsOpen, pendingCount }: {
  ...
  return (
    <header style={{
      ...
      borderBottom: '1px solid rgba(27,24,48,0.06)',
      ...
      <a href="/lobby" style={{ display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none', flexShrink: 0, zIndex: 2 }}>
        <div style={{
          width: 32, height: 32, borderRadius: 9, background: '#312E81',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, zIndex: 2 }}>
        ...
        <div style={{
          ...
          background: 'rgba(252,211,77,0.35)', border: '1px solid rgba(245,158,11,0.25)',
...
const QuickPlayCard = ({ onQuickPlay, searching, onJoin }: {
  ...
  return (
    <div style={{
      ...
      boxShadow: '0 1px 0 rgba(255,255,255,0.9) inset, 0 20px 40px -12px rgba(31,27,92,0.16)',
      borderRadius: 20, padding: '24px 28px',
      display: 'flex', flexDirection: 'column',
      position: 'relative', overflow: 'hidden',
      gridRow: 'span 2', // spans both rows in the grid
    }}>
      {/* Tape */}
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20, position: 'relative' }}>
        <button onClick={onQuickPlay} style={{
          ...
          <span style={{
            ...
            <span style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.2em', opacity: 0.6, marginTop: 2 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 20 }}>
        {[
          { icon: '🟢', label: '184 rooms', sub: 'open now' },
          { icon: '⚡', label: '< 5 sec', sub: 'wait time' },
          { icon: '🎯', label: '8 players', sub: '5 rounds' },
        ].map(s => (
          <div key={s.label} style={{
            background: 'rgba(255,255,255,0.6)', borderRadius: 12, padding: '10px 8px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            <div style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#5A5275' }}>
            ...
            <input
              ...
              style={{
                ...
                borderRadius: 12, fontSize: 13, color: '#1B1830',
        ...
        <button
          onClick={() => roomId.trim() && onJoin(roomId.trim(), password)}
          disabled={!roomId.trim()}
          style={{
            width: '100%', padding: '11px 0',
            background: roomId.trim() ? 'rgba(49,46,129,0.1)' : 'rgba(27,24,48,0.04)',
            border: `1px solid ${roomId.trim() ? 'rgba(49,46,129,0.25)' : 'rgba(27,24,48,0.08)'}`,
            borderRadius: 12, cursor: roomId.trim() ? 'pointer' : 'not-allowed',
            fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 15,
            color: roomId.trim() ? '#312E81' : '#5A5275',
...
const CustomRoomCard = ({ onCreateRoom }: { onCreateRoom: (s: RoomSettings) => void }) => {
  const [settings, setSettings] = useState<RoomSettings>({
    name: 'my sketchbook', maxPlayers: 8, rounds: 5, drawTime: 60, isPrivate: false, password: '',
  })
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setSettings(s => ({ ...s, name: ['maple\'s atelier', 'sketchy biz', 'doodle zone', 'art attack'][Math.floor(Math.random() * 4)] }))}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#5A5275', padding: 4 }}>
      ...
      <div>
        ...
        <input type="range" min={30} max={120} step={10} value={settings.drawTime}
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{
              ...
              background: 'rgba(255,255,255,0.8)', border: '1px solid rgba(27,24,48,0.1)',
              borderRadius: 10, fontSize: 13, color: '#1B1830',
              outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box',
            }}
          />
        </div>
      ...
      <button
        onClick={async () => { setCreating(true); await onCreateRoom(settings); setCreating(false) }}
        disabled={creating}
        style={{
...
const WordOfDayCard = () => {
  ...
  const reveal = () => {
    ...
    WORD_OF_DAY.split('').forEach((_, i) => {
      setTimeout(() => setLetters(l => l.map((v, idx) => idx === i ? true : v)), i * 110)
    })
  }
    ...
    <div style={{
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 14 }}>
          {WORD_OF_DAY.split('').map((ch, i) => (
            <div key={i} style={{
              ...
              background: letters[i] ? 'rgba(245,158,11,0.22)' : 'rgba(255,255,255,0.07)',
              border: `1px solid ${letters[i] ? 'rgba(245,158,11,0.45)' : 'rgba(255,255,255,0.1)'}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontFamily: "'Fredoka', sans-serif", fontWeight: 700, fontSize: 18, color: '#FBF6EC',
              transition: 'all 0.3s', opacity: letters[i] ? 1 : 0.3,
            }}>
              {letters[i] ? ch : '?'}
            </div>
          ))}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
          {['7 letters', 'noun', 'resets 14h'].map(t => (
            <span key={t} style={{ background: 'rgba(255,255,255,0.09)', color: 'rgba(251,246,236,0.6)', fontSize: 10, padding: '3px 8px', borderRadius: 999 }}>{t}</span>
          ))}
        </div>
      </div>

      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <span style={{ background: 'rgba(245,158,11,0.18)', color: 'rgba(252,211,77,0.95)', fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 999 }}>
          +500 ink bonus
        </span>
        <button onClick={reveal} disabled={revealed} style={{
          background: revealed ? 'rgba(255,255,255,0.08)' : 'rgba(245,158,11,0.9)',
          ...
          transition: 'all 0.2s',
          boxShadow: revealed ? 'none' : '0 3px 0 -1px rgba(160,100,0,0.6)',
        }}>
...
export default function LobbyClient({ user }: Props) {
  ...
  return (
    <div style={{
      ...
      backgroundImage: `radial-gradient(rgba(27,24,48,0.032) 1px, transparent 1px), radial-gradient(rgba(27,24,48,0.022) 1px, transparent 1px)`,
      ...
      <main style={{
        ...
        <div style={{ flexShrink: 0 }}>
          <div style={{ fontFamily: "'Caveat', cursive", color: '#F59E0B', fontSize: 18, lineHeight: 1, marginBottom: 1 }}>
            welcome back, {user.username} —
          </div>
          <h1 style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: '#1B1830', fontSize: 'clamp(1.5rem, 2.5vw, 2.2rem)', margin: 0, lineHeight: 1.1 }}>
        ...
        <div style={{
          ...
          {/* Create Room — top right */}
          <CustomRoomCard onCreateRoom={handleCreateRoom} />

          {/* Word of Day — bottom right */}
          <WordOfDayCard />
        </div>
      </main>
      ...
      <footer style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        fontSize: 11, color: '#5A5275',
        borderTop: '1px solid rgba(27,24,48,0.06)',
      }}>
