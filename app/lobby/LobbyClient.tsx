...
const TopNav = ({ user, onLogout, onFriendsOpen, pendingCount, invites, onInvitesOpen }: {
  ...
  <header className="lobby-header" style={{ width: '100%', padding: '0 32px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', flexShrink: 0, borderBottom: '1px solid rgba(27,24,48,0.06)', background: 'rgba(251,246,236,0.85)', backdropFilter: 'blur(12px)', zIndex: 10 }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, zIndex: 2 }}>
      ...
      {/* Invites Button */}
      <button onClick={onInvitesOpen} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 5, background: invites.length > 0 ? '#312E81' : 'rgba(255,255,255,0.7)', border: '1px solid rgba(27,24,48,0.1)', borderRadius: 999, padding: '5px 12px', fontSize: 12, fontWeight: 600, color: invites.length > 0 ? '#fff' : '#2A2545', cursor: 'pointer' }}>
        <Icon path={ICONS.bell} size={14} /> <span className="hide-on-mobile">Invites</span>
        {invites.length > 0 && (
...
export default function LobbyClient({ user }: Props) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      disconnectSocket() // <--- Fix: Disconnect when leaving Lobby to prevent GameRoom socket connection bug
    }
  }, [user.userId])
