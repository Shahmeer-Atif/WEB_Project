...
// ─── Top Nav (centered) ───────────────────────────────────────────────────────
const TopNav = ({ user, onLogout, onFriendsOpen, pendingCount, invites, onInvitesOpen }: {
  user: JWTPayload; onLogout: () => void; onFriendsOpen: () => void; pendingCount: number; invites: GameInvite[]; onInvitesOpen: () => void
}) => {
  const navItems = [
  ...
  return (
    <header style={{
      width: '100%', padding: '0 32px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      position: 'relative', flexShrink: 0, borderBottom: '1px solid rgba(27,24,48,0.06)', background: 'rgba(251,246,236,0.85)', backdropFilter: 'blur(12px)',
    }}>
      {/* Logo — left */}
      ...
      <nav style={{
        position: 'absolute', left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 2,
        background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(16px)', borderRadius: 999, padding: '4px 6px', border: '1px solid rgba(255,255,255,0.7)', boxShadow: '0 2px 12px rgba(31,27,92,0.08)',
      }}>
        {navItems.map(l => (
          <a key={l.label} href={l.href} style={{
            padding: '5px 16px', fontSize: 13, fontWeight: 600, borderRadius: 999, textDecoration: 'none',
            background: l.active ? '#1B1830' : 'transparent', color: l.active ? '#FBF6EC' : '#2A2545', transition: 'background 0.15s',
          }}>{l.label}</a>
        ))}
      </nav>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, zIndex: 2 }}>
        ...
        {/* Game Invites */}
        <button onClick={onInvitesOpen} style={{
          position: 'relative', display: 'flex', alignItems: 'center', gap: 5,
          background: invites.length > 0 ? '#312E81' : 'rgba(255,255,255,0.7)', border: '1px solid rgba(27,24,48,0.1)',
          borderRadius: 999, padding: '5px 12px', fontSize: 12, fontWeight: 600, color: invites.length > 0 ? '#fff' : '#2A2545', cursor: 'pointer',
        }}>
          <Icon path={ICONS.bell} size={14} /> Invites
          {invites.length > 0 && (
            <span style={{
              position: 'absolute', top: -4, right: -4, width: 16, height: 16, borderRadius: '50%',
              background: '#EC4899', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>{invites.length}</span>
          )}
        </button>
        ...
        <button onClick={onFriendsOpen} style={{
          position: 'relative', display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(27,24,48,0.1)',
          borderRadius: 999, padding: '5px 12px', fontSize: 12, fontWeight: 600, color: '#2A2545', cursor: 'pointer',
        }}>
          👥 Friends
          {pendingCount > 0 && (
            <span style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(27,24,48,0.1)', borderRadius: 999, padding: '3px 10px 3px 3px' }}>
          <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#312E81', color: '#FBF6EC', fontFamily: "'Fredoka', sans-serif", fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
// ─── Quick Play Card ──────────────────────────────────────────────────────────
const QuickPlayCard = ({ onQuickPlay, searching, onJoin }: {
  onQuickPlay: () => void; searching: boolean; onJoin: (id: string, pw?: string) => void
}) => {
  ...
  return (
    <div style={{
      background: 'linear-gradient(180deg, rgba(255,255,255,0.84), rgba(255,255,255,0.58))', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
      border: '1px solid rgba(255,255,255,0.65)', boxShadow: '0 1px 0 rgba(255,255,255,0.9) inset, 0 20px 40px -12px rgba(31,27,92,0.16)',
      borderRadius: 20, padding: '24px 28px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', gridRow: 'span 2',
    }}>
      {/* Tape & Blob */}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input value={roomId} onChange={e => setRoomId(e.target.value)} placeholder="Room ID  e.g. room-abc123" style={{ width: '100%', paddingLeft: 32, paddingRight: 12, paddingTop: 10, paddingBottom: 10, background: 'rgba(255,255,255,0.8)', border: '1px solid rgba(27,24,48,0.1)', borderRadius: 12, fontSize: 13, color: '#1B1830', outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }} />
          </div>
          <div style={{ flex: 1, position: 'relative' }}>
            <div style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#5A5275' }}><Icon path={ICONS.key} size={15} /></div>
            <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Password (if private)" style={{ width: '100%', paddingLeft: 32, paddingRight: 12, paddingTop: 10, paddingBottom: 10, background: 'rgba(255,255,255,0.8)', border: '1px solid rgba(27,24,48,0.1)', borderRadius: 12, fontSize: 13, color: '#1B1830', outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }} />
          </div>
        </div>
        <button onClick={() => roomId.trim() && onJoin(roomId.trim(), password)} disabled={!roomId.trim() || searching} style={{ width: '100%', padding: '11px 0', background: roomId.trim() ? 'rgba(49,46,129,0.1)' : 'rgba(27,24,48,0.04)', border: `1px solid ${roomId.trim() ? 'rgba(49,46,129,0.25)' : 'rgba(27,24,48,0.08)'}`, borderRadius: 12, cursor: roomId.trim() ? 'pointer' : 'not-allowed', fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 15, color: roomId.trim() ? '#312E81' : '#5A5275', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.15s' }}>
          <Icon path={ICONS.door} size={16} /> {searching ? 'Checking...' : 'Join this room'}
        </button>
      </div>
    </div>
  )
...
const CustomRoomCard = ({ onCreateRoom }: { onCreateRoom: (s: RoomSettings) => void }) => {
  ...
  const Chip = ({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) => (
    <button onClick={onClick} style={{ padding: '5px 11px', borderRadius: 999, fontSize: 12, fontWeight: 600, border: active ? '1px solid #1B1830' : '1px solid rgba(27,24,48,0.12)', background: active ? '#1B1830' : 'rgba(255,255,255,0.7)', color: active ? '#FBF6EC' : '#5A5275', cursor: 'pointer', transition: 'all 0.15s', boxShadow: active ? '0 2px 0 -1px #000' : 'none' }}>{children}</button>
  )
  ...
  return (
    <div style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.84), rgba(255,255,255,0.58))', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.65)', boxShadow: '0 1px 0 rgba(255,255,255,0.9) inset, 0 20px 40px -12px rgba(31,27,92,0.16)', borderRadius: 20, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div><div style={{ fontSize: 10, fontWeight: 700, color: '#5A5275', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>Players</div><div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>{[4, 6, 8, 12].map(n => <Chip key={n} active={settings.maxPlayers === n} onClick={() => setSettings(s => ({ ...s, maxPlayers: n }))}>{n}</Chip>)}</div></div>
        <div><div style={{ fontSize: 10, fontWeight: 700, color: '#5A5275', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>Rounds</div><div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>{[3, 5, 8, 10].map(n => <Chip key={n} active={settings.rounds === n} onClick={() => setSettings(s => ({ ...s, rounds: n }))}>{n}</Chip>)}</div></div>
      </div>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><div style={{ fontSize: 10, fontWeight: 700, color: '#5A5275', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Draw time</div><span style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: '#1B1830', fontSize: 14 }}>{settings.drawTime}s</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, padding: 3, background: 'rgba(27,24,48,0.05)', borderRadius: 12, position: 'relative' }}>
        ...
        {[{ label: '🌐 Public', value: false }, { label: '🔒 Private', value: true }].map(opt => (
          <button key={String(opt.value)} onClick={() => setSettings(s => ({ ...s, isPrivate: opt.value }))} style={{ position: 'relative', zIndex: 1, padding: '7px 0', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 13, color: settings.isPrivate === opt.value ? '#312E81' : '#5A5275', transition: 'color 0.2s' }}>{opt.label}</button>
        ))}
      </div>
      {settings.isPrivate && (
      ...
      <button onClick={async () => { setCreating(true); await onCreateRoom(settings); setCreating(false) }} disabled={creating} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: creating ? '#5A5275' : '#312E81', color: '#FBF6EC', border: 'none', cursor: creating ? 'not-allowed' : 'pointer', fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 16, padding: '12px 0', borderRadius: 14, boxShadow: creating ? 'none' : '0 5px 0 -1px #1F1B5C', transition: 'background 0.2s', marginTop: 2 }}>
        <Icon path={ICONS.plus} size={16} /> {creating ? 'Opening…' : 'Open the room'} {!creating && <Icon path={ICONS.arrow} size={16} />}
      </button>
    </div>
  )
}
    ...
    <div style={{ background: 'linear-gradient(135deg, #312E81 0%, #1e1b4b 100%)', borderRadius: 20, padding: '20px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', flex: 1 }}>
      ...
      <div style={{ position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
          <Icon path={ICONS.sparkle} size={12} /> <span style={{ fontFamily: "'Caveat', cursive", color: 'rgba(252,211,77,0.9)', fontSize: 16 }}>word of the day</span>
        </div>
        <div style={{ fontSize: 11, color: 'rgba(251,246,236,0.5)', marginBottom: 14 }}>{WORD_HINT}</div>
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 14 }}>
          {WORD_OF_DAY.split('').map((ch, i) => (
            <div key={i} style={{ width: 34, height: 40, borderRadius: 8, background: letters[i] ? 'rgba(245,158,11,0.22)' : 'rgba(255,255,255,0.07)', border: `1px solid ${letters[i] ? 'rgba(245,158,11,0.45)' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Fredoka', sans-serif", fontWeight: 700, fontSize: 18, color: '#FBF6EC', transition: 'all 0.3s', opacity: letters[i] ? 1 : 0.3 }}>{letters[i] ? ch : '?'}</div>
          ))}
        </div>
      </div>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        ...
        <button onClick={reveal} disabled={revealed} style={{ background: revealed ? 'rgba(255,255,255,0.08)' : 'rgba(245,158,11,0.9)', color: revealed ? 'rgba(255,255,255,0.45)' : '#1B1830', border: 'none', borderRadius: 10, padding: '8px 16px', fontSize: 13, fontWeight: 700, cursor: revealed ? 'default' : 'pointer', fontFamily: "'Fredoka', sans-serif", transition: 'all 0.2s', boxShadow: revealed ? 'none' : '0 3px 0 -1px rgba(160,100,0,0.6)' }}>{revealed ? '✓ Revealed!' : '👁 Reveal'}</button>
      </div>
    </div>
  )
}
...
export default function LobbyClient({ user }: Props) {
  ...
  // Setup Socket to listen for invites
  useEffect(() => {
    const socket = getSocket()
    socket.connect()
    
    // Let the server know we are online for invites
    ...
    socket.on('invite:receive', handleInvite)

    return () => {
      socket.off('invite:receive', handleInvite)
    }
  }, [user.userId])

  useEffect(() => {
    fetch('/api/friends').then(r => r.json()).then(d => setPendingCount(d.pendingReceived?.length || 0)).catch(() => {})
  }, [])
  ...
  return (
    <div style={{ height: '100vh', overflow: 'hidden', backgroundColor: '#FBF6EC', backgroundImage: `radial-gradient(rgba(27,24,48,0.032) 1px, transparent 1px), radial-gradient(rgba(27,24,48,0.022) 1px, transparent 1px)`, backgroundSize: '3px 3px, 7px 7px', backgroundPosition: '0 0, 1px 1px', fontFamily: "'Inter', system-ui, sans-serif", display: 'flex', flexDirection: 'column' }}>
      <TopNav user={user} onLogout={handleLogout} onFriendsOpen={() => setFriendsOpen(true)} pendingCount={pendingCount} invites={invites} onInvitesOpen={() => setInvitesOpen(!invitesOpen)} />

      {/* Invites Dropdown Modal */}
      {invitesOpen && (
        <div style={{ position: 'absolute', top: 65, right: 32, width: 300, background: '#fff', borderRadius: 16, boxShadow: '0 10px 40px rgba(31,27,92,0.15)', border: '1px solid rgba(27,24,48,0.08)', zIndex: 100, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(27,24,48,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#FBF6EC' }}>
            <span style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 700, color: '#1B1830', fontSize: 15 }}>Game Invites</span>
            <button onClick={() => setInvites([])} style={{ fontSize: 11, color: '#5A5275', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Clear All</button>
          </div>
          <div style={{ maxHeight: 300, overflowY: 'auto' }}>
            {invites.length === 0 ? (
              <div style={{ padding: 24, textAlign: 'center', color: '#5A5275', fontSize: 13, fontStyle: 'italic' }}>No pending invites.</div>
            ) : (
              invites.map((inv) => (
                <div key={inv.id} style={{ padding: '12px 16px', borderBottom: '1px solid rgba(27,24,48,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: 13, color: '#1B1830', fontWeight: 600 }}>@{inv.fromUsername}</div>
                    <div style={{ fontSize: 11, color: '#5A5275' }}>Invited you to play!</div>
                  </div>
                  <button onClick={() => { handleJoin(inv.roomId); setInvites(prev => prev.filter(i => i.id !== inv.id)); setInvitesOpen(false) }} style={{ background: '#312E81', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Join</button>
                </div>
              ))
            )}
          </div>
      ...
      <main style={{ flex: 1, minHeight: 0, padding: '14px 32px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr auto', gap: 14 }}>
          <div style={{ gridRow: 'span 2', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <QuickPlayCard onQuickPlay={handleQuickPlay} searching={searching} onJoin={handleJoin} />
          </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; } input[type=range] { cursor: pointer; }
        @media (max-width: 720px) { div[style*="gridTemplateColumns: 1fr 1fr"] { grid-template-columns: 1fr !important; grid-template-rows: auto !important; overflow-y: auto !important; } div[style*="gridRow: span 2"] { grid-row: span 1 !important; } body { overflow: auto !important; } div[style*="height: 100vh"] { height: auto !important; overflow: auto !important; } main { padding: 12px 16px !important; } header { padding: 0 16px !important; } footer { padding: 10px 16px !important; } }
      `}</style>
    </div>
  )
}