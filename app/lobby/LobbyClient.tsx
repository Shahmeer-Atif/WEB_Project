...
// Word of day is fetched dynamically from API
    ...
    <div style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.84), rgba(255,255,255,0.58))', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.65)', boxShadow: '0 1px 0 rgba(255,255,255,0.9) inset, 0 20px 40px -12px rgba(31,27,92,0.16)', borderRadius: 20, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {settings.isPrivate && (
        <div style={{ position: 'relative' }}>
          ...
          <input value={settings.password} onChange={e => setSettings(s => ({ ...s, password: e.target.value }))} type="password" placeholder="Set room password (required for private)" style={{ width: '100%', paddingLeft: 30, paddingRight: 12, paddingTop: 9, paddingBottom: 9, background: 'rgba(255,255,255,0.8)', border: '1px solid rgba(27,24,48,0.1)', borderRadius: 10, fontSize: 13, color: '#1B1830', outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }} />
        </div>
      )}
...
const WordOfDayCard = () => {
  const [wod, setWod] = useState({ word: '', hint: '', bonusPoints: 500 })
  const [revealed, setRevealed] = useState(false)
  const [letters, setLetters] = useState<boolean[]>([])

  useEffect(() => {
    fetch('/api/wordofday').then(r => r.json()).then(d => {
      setWod(d)
      setLetters(Array(d.word.length).fill(false))
    }).catch(() => {
      setWod({ word: 'PARADOX', hint: 'A statement that contradicts itself', bonusPoints: 500 })
      setLetters(Array(7).fill(false))
    })
  }, [])

  const reveal = () => {
    if (revealed || !wod.word) return; setRevealed(true)
    wod.word.split('').forEach((_, i) => { setTimeout(() => setLetters(l => l.map((v, idx) => idx === i ? true : v)), i * 110) })
  }

  if (!wod.word) return (
    <div style={{ background: 'linear-gradient(135deg, #312E81 0%, #1e1b4b 100%)', borderRadius: 20, padding: '20px 24px', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <span style={{ fontFamily: "'Caveat',cursive", color: 'rgba(252,211,77,0.7)', fontSize: 18 }}>Loading…</span>
    </div>
  )
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #312E81 0%, #1e1b4b 100%)', borderRadius: 20, padding: '20px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden', flex: 1 }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ fontSize: 11, color: 'rgba(251,246,236,0.5)', marginBottom: 14 }}>{wod.hint || 'Can you guess it?'}</div>
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 14 }}>
          {wod.word.split('').map((ch, i) => (
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
          {[`${wod.word.length} letters`, 'resets daily'].map(t => <span key={t} style={{ background: 'rgba(255,255,255,0.09)', color: 'rgba(251,246,236,0.6)', fontSize: 10, padding: '3px 8px', borderRadius: 999 }}>{t}</span>)}
        </div>
      </div>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <span style={{ background: 'rgba(245,158,11,0.18)', color: 'rgba(252,211,77,0.95)', fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 999 }}>+{wod.bonusPoints} ink bonus</span>
